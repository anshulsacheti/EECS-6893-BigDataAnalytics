...
def get_replay_pack(client_version, client_key, client_secret, replays_dir, extract=False):
    try:
        ...
        print('Searching replay packs with client version=%s' % client_version)
        ...
        print('Building urls for downloading replay packs. num_files=%d' % len(meta_file_urls))
        download_urls=[]
        ...
        for archive_url in sorted(download_urls):
            print('Downloading replay packs. url=%s' % archive_url)
        ...
        import traceback
        print('Failed to download replay packs. traceback=%s' % traceback.format_exc())
